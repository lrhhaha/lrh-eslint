...
var espree_default = () => (Parser3) => {
  ...
  return class Espree extends Parser3 {
    ...
    parse() {
      const extra = this[STATE];
      const program2 = super.parse();
      program2.sourceType = extra.originalSourceType;
      if (extra.comments) {
        program2.comments = extra.comments;
      }
      if (extra.tokens) {
        program2.tokens = extra.tokens;
      }
      if (program2.body.length) {
        const [firstNode] = program2.body;
        if (program2.range) {
          program2.range[0] = firstNode.range[0];
        }
        if (program2.loc) {
          program2.loc.start = firstNode.loc.start;
        }
        program2.start = firstNode.start;
      }
      if (extra.lastToken) {
        if (program2.range) {
          program2.range[1] = extra.lastToken.range[1];
        }
        if (program2.loc) {
          program2.loc.end = extra.lastToken.loc.end;
        }
        program2.end = extra.lastToken.end;
      }
...
// src/linter.ts
var import_estraverse = __toESM(require_estraverse());
...
// src/rules/index.ts
var noUnusedVarsListeners = no_unused_vars_default.create(null);
var array = [noUnusedVarsListeners];
...
function getCode(path2) {
  try {
    const data2 = import_node_fs.default.readFileSync(path2, "utf8");
    return data2;
  } catch (err) {
  ...
  import_estraverse.default.traverse(ast, {
    ...
    leave: function(node) {
      if (node.type === "Program") {
        const leaveFns = rules_default.get("output");
