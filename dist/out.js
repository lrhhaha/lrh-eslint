...
var semi_default = {
  ...
  create(ctx) {
    ...
    return {
      ...
      // 所有节点
      all: function(node, parent) {
        const { body, type } = node;
        if (body === void 0) return;
        if (type === "Program") tokens = node.tokens;
        body.forEach((node2) => {
          const { end } = node2;
          const token = tokens.find((it) => it.end === end);
          if (token) {
            const { type: type2, value } = token;
            if (type2 !== "Punctuator" || value !== ";") {
              reports.push({
                // node,
...
function getCode(path2) {
  try {
    const data2 = import_node_fs2.default.readFileSync(path2, "utf8");
    return data2;
  } catch (err) {
